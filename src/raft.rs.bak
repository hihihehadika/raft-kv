...
pub struct RaftNode {
    ...
    /// log[0] is a sentinel entry (term=0) so prevLogIndex=0 is always valid.
...
impl RaftNode {
    ...
    fn become_leader(&mut self) -> Vec<OutboundMsg> {
        ...
        let next = self.log.len(); // one past our last entry
    ...
    pub fn handle_append_entries(
        ...
        // Rule 2: reject if our log is missing prevLogIndex, or the term there
        // doesn't match prevLogTerm (log consistency check)
        if prev_idx >= self.log.len() || self.log[prev_idx].term != args.prev_log_term {
            return reject(self.current_term);
        }

        // Rules 3 & 4: merge entries into our log
        for (offset, new_entry) in args.entries.iter().enumerate() {
            let idx = prev_idx + 1 + offset;
            if idx < self.log.len() {
                ...
                // else: already have this identical entry, skip
            } else {
                self.log.push(new_entry.clone());
            }
        }
        ...
        if args.leader_commit > self.commit_index as u64 {
            self.commit_index =
                (args.leader_commit as usize).min(self.log.len() - 1);
        }

        let match_index = (prev_idx + args.entries.len()) as u64;
        (
    ...
    fn try_advance_commit_index(&mut self) {
        ...
        for n in ((self.commit_index + 1)..=last).rev() {
            ...
            let replicated = self.match_index.values().filter(|&&mi| mi >= n).count() + 1; // +1 = self
    ...
    pub fn apply_committed(&mut self, store: &mut KvStore) -> usize {
        ...
        while self.last_applied < self.commit_index {
            self.last_applied += 1;
            match self.log[self.last_applied].command.clone() {
